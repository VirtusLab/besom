...
//      scalaCli.publishLocal(result.outputDir).call()
    ...
    teardown = pulumi.fixture.teardown
  ).test("kubernetes provider should work with secrets".only /* FIXME: remove before merge */) { ctx =>
    val result = pulumi.up(ctx.stackName, "--skip-preview").call(cwd = ctx.programDir, env = ctx.env, check = false)
    if result.exitCode != 0 then
      println(result.out.text())
      println(result.err.text())
      fail(s"pulumi up failed with exit code ${result.exitCode}")

    val y1 = os.read(ctx.programDir / "output" / "1-manifest" / "v1-secret-default-test-secret1.yaml")
    assertEquals(
      y1,
      """apiVersion: v1
          |data:
    ...
    val v2 = os.read(ctx.programDir / "output" / "1-manifest" / "v1-secret-default-test-secret2.yaml")
    assertEquals(
      v2,
      """apiVersion: v1
          |data:
    ...
    val y3 = os.read(ctx.programDir / "output" / "1-manifest" / "v1-secret-default-test-secret3.yaml")
    assertEquals(
      y3,
      """apiVersion: v1
          |data: {}
