...
trait ParserInput {
  ...
  /** Advance the cursor and get the next char, which could potentially be outside of the 7-Bit ASCII range. Therefore decoding might be
    * required.
    */
...
object ParserInput {
  ...
  /** ParserInput reading directly off a byte array which is assumed to contain the UTF-8 encoded representation of the JSON input, without
    * requiring a separate decoding step.
    */
