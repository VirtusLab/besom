...
/** Regression tests for the inference of the type parameter of the `Input` opaque types.
  *
  * `Input[A]`, `Input.Optional[A]` and `Input.OneOrIterable[A]` are unions, and the alternative
  * naming the bare `A` used to come first. When the argument is an application - which every
  * resource constructor is - the solver matched that leftmost alternative directly and instantiated
  * `A := Output[R]` instead of `A := R`. The result typechecked, but `wrappedAsOptionOutput` then
  * took its `case output: Output[_]` arm (an erased class test that cannot tell a bare `A` that
  * happens to be an `Output` from an `Output` wrapper), stripped a level of wrapping that was never
  * there, and produced a value whose runtime shape contradicted its static type. Any later use of
  * the element as an `Output` then threw a ClassCastException.
  *
  * The unions are now ordered wrapper-first, which is the same type - union subtyping is
  * commutative - but stops the solver taking that shortcut.
  *
  * Two things matter about how these tests are written, and both are why the pre-existing
  * `Output.when` cases in OutputTest never caught this:
  *
  *   - every argument is passed as an APPLICATION, never bound to a `val` first. A stable
  *     identifier infers correctly even on the old ordering.
  *   - each result is bound WITHOUT an expected type and asserted on the following line. Ascribing
  *     at the definition would drive inference and hide what is under test.
  */
