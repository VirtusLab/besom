...
class OutputTest extends munit.FunSuite:
  ...
  def takesAnOptionalList(list: Input.Optional[List[Input[String]]])(
    using Context
  ): Output[Option[List[String]]] =
    list.asOptionOutput()

  def takesAMap(map: Input[Map[String, Input[String]]])(
    using Context
  ): Output[Map[String, String]] =
    map.asOutput()

  def takesAnOptionalMap(map: Input.Optional[Map[String, Input[String]]])(
    using Context
  ...
  test("multi-input map type functions") {
    ...
    assert(takesAnOptionalMap(Output(Option(Map("key" -> Output("value"))))).getData.unsafeRunSync() == OutputData(Option(Map("key" -> "value"))))
