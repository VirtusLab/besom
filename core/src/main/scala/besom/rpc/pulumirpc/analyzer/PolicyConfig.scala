// Generated by the Scala Plugin for the Protocol Buffer Compiler.
// Do not edit!
//
// Protofile syntax: PROTO3

package pulumirpc.analyzer

/** PolicyConfig provides configuration for a policy.
  *
  * @param enforcementLevel
  *   Enforcement level of the policy.
  * @param properties
  *   Configuration properties of the policy.
  */
@SerialVersionUID(0L)
final case class PolicyConfig(
    enforcementLevel: pulumirpc.analyzer.EnforcementLevel = pulumirpc.analyzer.EnforcementLevel.ADVISORY,
    properties: _root_.scala.Option[com.google.protobuf.struct.Struct] = _root_.scala.None,
    unknownFields: _root_.scalapb.UnknownFieldSet = _root_.scalapb.UnknownFieldSet.empty
    ) extends scalapb.GeneratedMessage with scalapb.lenses.Updatable[PolicyConfig] {
    @transient
    private[this] var __serializedSizeMemoized: _root_.scala.Int = 0
    private[this] def __computeSerializedSize(): _root_.scala.Int = {
      var __size = 0
      
      {
        val __value = enforcementLevel.value
        if (__value != 0) {
          __size += _root_.com.google.protobuf.CodedOutputStream.computeEnumSize(1, __value)
        }
      };
      if (properties.isDefined) {
        val __value = properties.get
        __size += 1 + _root_.com.google.protobuf.CodedOutputStream.computeUInt32SizeNoTag(__value.serializedSize) + __value.serializedSize
      };
      __size += unknownFields.serializedSize
      __size
    }
    override def serializedSize: _root_.scala.Int = {
      var __size = __serializedSizeMemoized
      if (__size == 0) {
        __size = __computeSerializedSize() + 1
        __serializedSizeMemoized = __size
      }
      __size - 1
      
    }
    def writeTo(`_output__`: _root_.com.google.protobuf.CodedOutputStream): _root_.scala.Unit = {
      {
        val __v = enforcementLevel.value
        if (__v != 0) {
          _output__.writeEnum(1, __v)
        }
      };
      properties.foreach { __v =>
        val __m = __v
        _output__.writeTag(2, 2)
        _output__.writeUInt32NoTag(__m.serializedSize)
        __m.writeTo(_output__)
      };
      unknownFields.writeTo(_output__)
    }
    def withEnforcementLevel(__v: pulumirpc.analyzer.EnforcementLevel): PolicyConfig = copy(enforcementLevel = __v)
    def getProperties: com.google.protobuf.struct.Struct = properties.getOrElse(com.google.protobuf.struct.Struct.defaultInstance)
    def clearProperties: PolicyConfig = copy(properties = _root_.scala.None)
    def withProperties(__v: com.google.protobuf.struct.Struct): PolicyConfig = copy(properties = Option(__v))
    def withUnknownFields(__v: _root_.scalapb.UnknownFieldSet) = copy(unknownFields = __v)
    def discardUnknownFields = copy(unknownFields = _root_.scalapb.UnknownFieldSet.empty)
    def getFieldByNumber(__fieldNumber: _root_.scala.Int): _root_.scala.Any = {
      (__fieldNumber: @_root_.scala.unchecked) match {
        case 1 => {
          val __t = enforcementLevel.javaValueDescriptor
          if (__t.getNumber() != 0) __t else null
        }
        case 2 => properties.orNull
      }
    }
    def getField(__field: _root_.scalapb.descriptors.FieldDescriptor): _root_.scalapb.descriptors.PValue = {
      _root_.scala.Predef.require(__field.containingMessage eq companion.scalaDescriptor)
      (__field.number: @_root_.scala.unchecked) match {
        case 1 => _root_.scalapb.descriptors.PEnum(enforcementLevel.scalaValueDescriptor)
        case 2 => properties.map(_.toPMessage).getOrElse(_root_.scalapb.descriptors.PEmpty)
      }
    }
    def toProtoString: _root_.scala.Predef.String = _root_.scalapb.TextFormat.printToUnicodeString(this)
    def companion: pulumirpc.analyzer.PolicyConfig.type = pulumirpc.analyzer.PolicyConfig
    // @@protoc_insertion_point(GeneratedMessage[pulumirpc.PolicyConfig])
}

object PolicyConfig extends scalapb.GeneratedMessageCompanion[pulumirpc.analyzer.PolicyConfig] {
  implicit def messageCompanion: scalapb.GeneratedMessageCompanion[pulumirpc.analyzer.PolicyConfig] = this
  def parseFrom(`_input__`: _root_.com.google.protobuf.CodedInputStream): pulumirpc.analyzer.PolicyConfig = {
    var __enforcementLevel: pulumirpc.analyzer.EnforcementLevel = pulumirpc.analyzer.EnforcementLevel.ADVISORY
    var __properties: _root_.scala.Option[com.google.protobuf.struct.Struct] = _root_.scala.None
    var `_unknownFields__`: _root_.scalapb.UnknownFieldSet.Builder = null
    var _done__ = false
    while (!_done__) {
      val _tag__ = _input__.readTag()
      _tag__ match {
        case 0 => _done__ = true
        case 8 =>
          __enforcementLevel = pulumirpc.analyzer.EnforcementLevel.fromValue(_input__.readEnum())
        case 18 =>
          __properties = Option(__properties.fold(_root_.scalapb.LiteParser.readMessage[com.google.protobuf.struct.Struct](_input__))(_root_.scalapb.LiteParser.readMessage(_input__, _)))
        case tag =>
          if (_unknownFields__ == null) {
            _unknownFields__ = new _root_.scalapb.UnknownFieldSet.Builder()
          }
          _unknownFields__.parseField(tag, _input__)
      }
    }
    pulumirpc.analyzer.PolicyConfig(
        enforcementLevel = __enforcementLevel,
        properties = __properties,
        unknownFields = if (_unknownFields__ == null) _root_.scalapb.UnknownFieldSet.empty else _unknownFields__.result()
    )
  }
  implicit def messageReads: _root_.scalapb.descriptors.Reads[pulumirpc.analyzer.PolicyConfig] = _root_.scalapb.descriptors.Reads{
    case _root_.scalapb.descriptors.PMessage(__fieldsMap) =>
      _root_.scala.Predef.require(__fieldsMap.keys.forall(_.containingMessage eq scalaDescriptor), "FieldDescriptor does not match message type.")
      pulumirpc.analyzer.PolicyConfig(
        enforcementLevel = pulumirpc.analyzer.EnforcementLevel.fromValue(__fieldsMap.get(scalaDescriptor.findFieldByNumber(1).get).map(_.as[_root_.scalapb.descriptors.EnumValueDescriptor]).getOrElse(pulumirpc.analyzer.EnforcementLevel.ADVISORY.scalaValueDescriptor).number),
        properties = __fieldsMap.get(scalaDescriptor.findFieldByNumber(2).get).flatMap(_.as[_root_.scala.Option[com.google.protobuf.struct.Struct]])
      )
    case _ => throw new RuntimeException("Expected PMessage")
  }
  def javaDescriptor: _root_.com.google.protobuf.Descriptors.Descriptor = AnalyzerProto.javaDescriptor.getMessageTypes().get(11)
  def scalaDescriptor: _root_.scalapb.descriptors.Descriptor = AnalyzerProto.scalaDescriptor.messages(11)
  def messageCompanionForFieldNumber(__number: _root_.scala.Int): _root_.scalapb.GeneratedMessageCompanion[_] = {
    var __out: _root_.scalapb.GeneratedMessageCompanion[_] = null
    (__number: @_root_.scala.unchecked) match {
      case 2 => __out = com.google.protobuf.struct.Struct
    }
    __out
  }
  lazy val nestedMessagesCompanions: Seq[_root_.scalapb.GeneratedMessageCompanion[_ <: _root_.scalapb.GeneratedMessage]] = Seq.empty
  def enumCompanionForFieldNumber(__fieldNumber: _root_.scala.Int): _root_.scalapb.GeneratedEnumCompanion[_] = {
    (__fieldNumber: @_root_.scala.unchecked) match {
      case 1 => pulumirpc.analyzer.EnforcementLevel
    }
  }
  lazy val defaultInstance = pulumirpc.analyzer.PolicyConfig(
    enforcementLevel = pulumirpc.analyzer.EnforcementLevel.ADVISORY,
    properties = _root_.scala.None
  )
  implicit class PolicyConfigLens[UpperPB](_l: _root_.scalapb.lenses.Lens[UpperPB, pulumirpc.analyzer.PolicyConfig]) extends _root_.scalapb.lenses.ObjectLens[UpperPB, pulumirpc.analyzer.PolicyConfig](_l) {
    def enforcementLevel: _root_.scalapb.lenses.Lens[UpperPB, pulumirpc.analyzer.EnforcementLevel] = field(_.enforcementLevel)((c_, f_) => c_.copy(enforcementLevel = f_))
    def properties: _root_.scalapb.lenses.Lens[UpperPB, com.google.protobuf.struct.Struct] = field(_.getProperties)((c_, f_) => c_.copy(properties = Option(f_)))
    def optionalProperties: _root_.scalapb.lenses.Lens[UpperPB, _root_.scala.Option[com.google.protobuf.struct.Struct]] = field(_.properties)((c_, f_) => c_.copy(properties = f_))
  }
  final val ENFORCEMENTLEVEL_FIELD_NUMBER = 1
  final val PROPERTIES_FIELD_NUMBER = 2
  def of(
    enforcementLevel: pulumirpc.analyzer.EnforcementLevel,
    properties: _root_.scala.Option[com.google.protobuf.struct.Struct]
  ): _root_.pulumirpc.analyzer.PolicyConfig = _root_.pulumirpc.analyzer.PolicyConfig(
    enforcementLevel,
    properties
  )
  // @@protoc_insertion_point(GeneratedMessageCompanion[pulumirpc.PolicyConfig])
}
